...
// Volume esperado — carrega do localStorage se disponível
let EXPECTED_RESPONSES = JSON.parse(localStorage.getItem('odara_expected') || 'null') || { internos: 30, liderancas: 8, fornecedores: 10, distribuidores: 10 };
let EXPECTED_TOTAL = Object.values(EXPECTED_RESPONSES).reduce((a,b)=>a+b,0);

function saveExpected() {
  localStorage.setItem('odara_expected', JSON.stringify(EXPECTED_RESPONSES));
...
function buildHeatmapByArea(audience) {
  ...
  const rows = dimList.map(dim => {
    ...
    (DATA.rawResponses || []).forEach(r => {
      ...
      if (scoresByArea[area]) scoresByArea[area].push(val);
      else { scoresByArea['Geral'] = scoresByArea['Geral'] || []; scoresByArea['Geral'].push(val); }
    });
    ...
    // Only include if at least one area has data
    if (scores.every(s => s === null)) return null;
    return { name: dim.length > 20 ? dim.split(' ')[0] : dim, full: dim, scores };
  }).filter(Boolean);
...
function saveConfig() {
  ...
  saveExpected();
  document.getElementById('cfg-msg').textContent = '✅ Configuração salva! Taxa de conclusão atualizada.';
