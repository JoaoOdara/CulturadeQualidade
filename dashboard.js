...
async function loadFromSupabase() {
  ...
  try {
    ...
    // Guardamos o conjunto sem filtro para a aba de Auditoria. Só refiltramos
    // o array oficial quando há IDs a excluir — caso contrário deixamos como
    // está (caso comum: nenhum respondente marcado como `confirmed`).
    const allResponses = responses.slice();
    if (excludedIds.size > 0) {
      const filtered = responses.filter(r => !excludedIds.has(r.respondent_id));
      responses.length = 0;
      Array.prototype.push.apply(responses, filtered);
    }
