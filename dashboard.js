...
function initDimBarChart() {
  ...
  // Mostrar dimensões de internos (que é o público principal)
  const ks = DIM_NAMES.filter(d => DATA.dims[d] && (DATA.dims[d].internos !== null));
  const datasets = [
  ...
  // Adicionar lideranças se houver dados (com suas próprias dims)
  const hasLid = DIM_LID_NAMES.some(d => DATA.dims[d]?.liderancas !== null);
  if (hasLid) {
    // Para comparação visual, mostramos lideranças ao lado de internos nas dimensões em comum
    datasets.push({ label:'Lideranças', data:ks.map(d=>DATA.dims[d]?.liderancas||0), backgroundColor:C.p+'99', borderColor:C.p, borderWidth:1.5, borderRadius:3 });
  }
...
function renderDimTable() {
  ...
  // Helper to add rows for a set of dimensions and audience
  function addDimRows(dimList, audienceKey, audienceLabel) {
    dimList.forEach(dim => {
      const sc = DATA.dims[dim];
      if (!sc || sc[audienceKey] === null || sc[audienceKey] === undefined) return;
      rows.push({dim, pub:audienceLabel, score:sc[audienceKey], bold:true});
    });
  }

  // Internos
  addDimRows(DIM_NAMES, 'internos', 'Colaboradores');
  // Lideranças
  addDimRows(DIM_LID_NAMES, 'liderancas', 'Lideranças');
  // Fornecedores
  addDimRows(DIM_FORN_NAMES, 'fornecedores', 'Fornecedores');
  // Distribuidores
  addDimRows(DIM_DIST_NAMES, 'distribuidores', 'Distribuidores');
