...
// ══════════════════════════════════════════════
// HEATMAP — por público com breakdown por área
// ══════════════════════════════════════════════
function buildHeatmapByArea(audience) {
  if (!DATA || DATA.total === 0) return null;
  const dimMap = { internos:DIM_NAMES, liderancas:DIM_LID_NAMES, fornecedores:DIM_FORN_NAMES, distribuidores:DIM_DIST_NAMES };
  const dimList = dimMap[audience] || DIM_NAMES;

  // Build respondent→area lookup from segmentation data
  const respArea = {};
  (DATA.respondents || []).forEach(r => {
    if (r.survey_type === audience) {
      // seg_area = SEG2 (área/setor), seg_audience = SEG1 (público)
      respArea[r.id] = r.seg_area || 'Geral';
    }
  });
  console.info(`[Odara] Heatmap ${audience}: ${Object.keys(respArea).length} respondentes, áreas:`, [...new Set(Object.values(respArea))]);

  // Collect unique areas + always include 'Geral' as fallback
  let areas = [...new Set(Object.values(respArea))].filter(Boolean).sort();
  if (!areas.length) areas = ['Geral'];

  // If only one area or only 'Geral', show single-column (simpler)
  const singleCol = areas.length <= 1;

  // Build dimension scores per area
  ...
  const rows = dimList.map(dim => {
    ...
    rawResp.forEach(r => {
      if (r.survey_type !== audience) return;
      if (Q_DIM_MAP[r.question_id] !== dim) return;
      const val = parseFloat(r.value_numeric);
      if (isNaN(val) || val < 1 || val > 5) return;
  ...
  if (!rows.length) {
    // Fallback: tentar usando dados agregados de DATA.dims
    console.info(`[Odara] Heatmap ${audience}: sem dados por área, tentando fallback por dims`);
    const fallbackRows = dimList.map(d => {
      const sc = DATA.dims[d]?.[audience];
      return sc !== null && sc !== undefined ? { name: d.length > 25 ? d.split(' ')[0] : d, full: d, scores: [sc] } : null;
    }).filter(Boolean);
    if (fallbackRows.length) return { areas: ['Score geral'], rows: fallbackRows };
    return null;
  }
  return { areas, rows };
}
...
function renderHeatmap() {
  const audiences = [
    ...
    { key:'liderancas', id:null, label:'🏆 Lideranças', color:'#EE2737' },
  ...
  const intData = buildHeatmapByArea('internos');
  ...
  if (!hmLid) {
    ...
    if (hmFor) parent.insertBefore(hmLid, hmFor.closest('.card')?.parentElement || hmFor);
    else parent.appendChild(hmLid);
  }
  const lidData = buildHeatmapByArea('liderancas');
  ...
  // Fornecedores
  const hmFor = document.getElementById('hm-fornecedores');
  const forData = buildHeatmapByArea('fornecedores');
  ...
  const disData = buildHeatmapByArea('distribuidores');
...
function buildHmFilters() {
  ...
  if (!el) return;
  el.innerHTML = `<span style="font-size:9px;color:var(--g);">Scores por área de respondente · Clique em um quadrado para análise detalhada →</span>`;
}
...
function renderDrillDown() {
  ...
  if (!el) return;
  if (!heatSel) {
    el.innerHTML = `<div class="dd-hint">👈 Clique em qualquer célula do heatmap para ver as perguntas que compõem a dimensão e seus scores individuais.</div>`;
    return;
  }
  ...
  const qListHtml = qScores.length ? qScores.map(q => {
    const qsc = scoreClass(q.avg);
    const text = Q_TEXT_MAP[q.qid] || q.qid;
    return `<div class="dq-row">
      <div class="dq-id">${q.qid}</div>
      <div class="dq-text">${text}</div>
      <div class="dq-bar"><div class="dq-bt"><div class="dq-bf" style="width:${((q.avg-1)/4)*100}%;background:${heatColor(q.avg)}"></div></div></div>
      <span class="pill ${qsc.cls}" style="font-size:8px;min-width:35px;text-align:center;">${q.avg.toFixed(1)}</span>
    </div>`;
  }).join('') : '<div style="font-size:10px;color:var(--g);padding:8px;">Sem respostas para estas perguntas neste público.</div>';
  ...
  el.innerHTML = `<div class="dd-card">
    ...
    <div class="dd-body">
      <div>
        <div class="dd-st">📋 Perguntas da dimensão (${qScores.length})</div>
        <div style="max-height:320px;overflow-y:auto;">${qListHtml}</div>
      </div>
      <div>
        ...
        <div class="da-list">
          ${score < 3.5 ? `<div class="da da-crit">
            ...
            <div class="da-b">Score ${score.toFixed(1)} — Zona de intervenção. Plano em 30 dias.</div>
          </div>` : ''}
          <div class="da da-info">
            <div class="da-t">${dim}</div>
            <div class="da-b">${DIM_RECS[dim] || recAction(score)}</div>
