...
const ALL_DIM_NAMES = [...new Set([...DIM_NAMES, ...DIM_LID_NAMES])];
...
async function loadFromSupabase() {
  ...
  try {
    ...
    // 2. Tentar a view dimension_scores (depende de tabela questions estar populada)
    let dimRows = null;
    try {
    ...
    // 3. Todas as respostas numéricas + question_id (para fallback e distribuição)
    const responses = await window.sbSelect(
      '/responses?select=value_numeric,survey_type,question_id&value_numeric=not.is.null'
    );
    ...
    const dims = {};
    ALL_DIM_NAMES.forEach(d => { dims[d] = { geral:null, internos:null, liderancas:null }; });
    ...
    if (dimRows && dimRows.length > 0) {
      ...
      const buckets = {}; // { 'Liderança e prioridade': { internos: [4,5,3,...], liderancas: [...] } }
      (responses || []).forEach(r => {
        const dim = Q_DIM_MAP[r.question_id];
        if (!dim) return; // pergunta aberta ou não mapeada
        if (!buckets[dim]) buckets[dim] = { internos:[], liderancas:[] };
        const val = parseFloat(r.value_numeric);
        if (isNaN(val) || val < 1 || val > 5) return;
        if (r.survey_type === 'internos') buckets[dim].internos.push(val);
        else if (r.survey_type === 'liderancas') buckets[dim].liderancas.push(val);
      });
      Object.entries(buckets).forEach(([dim, data]) => {
        if (!dims[dim]) dims[dim] = { geral:null, internos:null, liderancas:null };
        if (data.internos.length)   dims[dim].internos   = parseFloat((data.internos.reduce((a,b)=>a+b,0)/data.internos.length).toFixed(2));
        if (data.liderancas.length) dims[dim].liderancas = parseFloat((data.liderancas.reduce((a,b)=>a+b,0)/data.liderancas.length).toFixed(2));
      });
    }
    ...
    const verbatim = (openResp || [])
      .filter(r => r.value && r.value.length > 10 && OPEN_IDS.has(r.question_id))
      .slice(0, 30)
      .map(r => {
        // Classificar: primeira aberta = forças, segunda = fraquezas, terceira = ações
        let type = 'actions';
        if (['I24','L19','F13','D13'].includes(r.question_id)) type = 'strengths';
        else if (['I25','L20','F14','D14'].includes(r.question_id)) type = 'weaknesses';
        return { type, aud: r.survey_type || 'Respondente', txt: r.value };
      });
...
function initRadarChart() {
  ...
  const ks = DIM_NAMES.filter(k => DATA.dims[k] && DATA.dims[k].geral !== null);
  ...
  // Só adiciona lideranças se houver dados reais (não zeros)
  const hasLid = ks.some(d => DATA.dims[d].liderancas !== null && DATA.dims[d].liderancas > 0);
  if (hasLid) {
...
function renderSegmented() {
  ...
  el.innerHTML = AUD.map(a => {
    ...
    const dimList = a.key === 'liderancas' ? DIM_LID_NAMES : DIM_NAMES;
...
function renderDimTable() {
  ...
  DIM_NAMES.forEach(dim => {
    ...
    if (!sc) return;
    const hasData = sc.internos !== null || sc.liderancas !== null;
    if (!hasData) return;
    if (sc.geral !== null) rows.push({dim, pub:'Geral',         score:sc.geral,     bold:true});
  ...
  // Liderancas-specific dimensions
  DIM_LID_NAMES.forEach(dim => {
    const sc = DATA.dims[dim];
    if (!sc || sc.liderancas === null) return;
    rows.push({dim, pub:'Lideranças', score:sc.liderancas, bold:true});
  });
...
function buildHeatmapDataFromDims(audience) {
  ...
  const dimList = audience === 'liderancas' ? DIM_LID_NAMES : DIM_NAMES;
...
function renderHeatmap() {
  ...
  hmInt.innerHTML = intData ? '<h4 style="font-size:11px;font-weight:700;margin-bottom:8px;color:var(--p);">👷 Colaboradores</h4>' + buildHmTable(intData, 'internos') : emptyState('Aguardando respostas de colaboradores.');

  // Lideranças — novo painel
  ...
  if (!hmLid) {
    ...
    parent.insertBefore(hmLid, document.getElementById('hm-fornecedores'));
  }
  const lidData = buildHeatmapDataFromDims('liderancas');
  hmLid.innerHTML = lidData ? '<h4 style="font-size:11px;font-weight:700;margin-bottom:8px;color:var(--r);">🏆 Lideranças</h4>' + buildHmTable(lidData, 'liderancas') : emptyState('Aguardando respostas de lideranças.');
...
function buildHmFilters() {
  ...
  el.innerHTML = `
    ...
    <select class="hm-sel" onchange="heatFilterAud=this.value;renderHeatmap()">
      ...
      <option value="internos">Colaboradores</option>
